...
%
    ...
    L_plus_1 = numel(Q_primes);            % = L + 1

    % 校验：ct1 和 ct2 必须有相同的层数
    if numel(ct1.c0) ~= L_plus_1 || numel(ct2.c0) ~= L_plus_1 ...
       || numel(ct1.c1) ~= L_plus_1 || numel(ct2.c1) ~= L_plus_1
        error('CKKS_Add: 输入 ciphertext 层数不一致，无法相加。');
    end

    % 预分配输出结构
    ct_out.c0 = cell(1, L_plus_1);
    ct_out.c1 = cell(1, L_plus_1);
